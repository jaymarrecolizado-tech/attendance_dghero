(function () {
  ...
  function applyFilter() {
    var rows = document.querySelectorAll('#vipTableBody tr[data-status]');
    var visible = 0;
    ...
    var empty = document.querySelector('#vipTableBody .seo-filter-empty');
    if (empty) empty.remove();
    if (rows.length && visible === 0) {
      var tr = document.createElement('tr');
      tr.className = 'seo-filter-empty';
      tr.innerHTML = '<td colspan="4"><div class="seo-empty">No VIP guests match this filter.</div></td>';
      document.getElementById('vipTableBody').appendChild(tr);
    }
  }
  ...
  function renderVipRows(vips) {
    ...
    body.innerHTML = vips.map(function (v) {
